...
pub fn decode(v: u32) ?Inst {
    const inst = switch (dec_opcode(v)) {
        ...
        OPCODE_COP1 => Inst{ .cop1 = j_type(v) },
        OPCODE_COP2 => switch (dec_cop_opcode(v)) {
            COP_OPCODE_MFC => Inst{ .mfc2 = r_type(v) },
            COP_OPCODE_CFC => Inst{ .cfc2 = r_type(v) },
            COP_OPCODE_MTC => Inst{ .mtc2 = r_type(v) },
            COP_OPCODE_CTC => Inst{ .ctc2 = r_type(v) },
            // else => switch (dec_cop_bc_opcode(v)) {
            //     COP_BC_OPCODE_BCF => Inst{ .bc0f = i_type(v) },
            //     COP_BC_OPCODE_BCT => Inst{ .bc0t = i_type(v) },
            //     else => null,
        ...
        OPCODE_COP3 => Inst{ .cop3 = j_type(v) },
